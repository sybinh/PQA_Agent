...
def example_nested_query():
    query = client.query(
        ...
        select=[
            IssueProperty.id,
            IssueProperty.belongstoproject.nested([ProjectProperty.id, ProjectProperty.rotitle]),
            IssueProperty.rotitle,
            IssueProperty.rotype,
            IssueProperty.cq__Description,
            IssueProperty.estimatedeffort,
        ],
        where=(
            IssueProperty.belongstoproject.nested(
                ProjectProperty.id == "RQONE03870937"
            )
        )
