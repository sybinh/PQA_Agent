...
class Rule_Bc_CheckPstDates:
    ...
    def execute(self) -> List[ValidationResult]:
        ...
        Returns:
            List of ValidationResult - one per PST violation (or single PASS result)
        """
        ...
        # Check all PST mappings - return one ValidationResult per violation
        results = []
        ...
        for pst_mapping in self.pst_mappings:
            ...
            if bc_planned_date > pst_req_date:
                days_late = (bc_planned_date - pst_req_date).days
                violation_msg = (
                    f"PST {pst_mapping.pst_id}: Requested delivery {pst_req_date.strftime('%Y-%m-%d')}, "
                    f"BC planned {bc_planned_date.strftime('%Y-%m-%d')} (BC is {days_late} days late)"
                )
                
                # Create separate ValidationResult for this PST
                results.append(ValidationResult(
                    passed=False,
                    severity="WARNING",
                    title=f"BC planned date too late for {pst_mapping.pst_type} {pst_mapping.pst_id}",
                    description=(
                        f"BC {bc_id} planned date ({bc_planned_date.strftime('%Y-%m-%d')}) "
                        f"is later than requested delivery date of {pst_mapping.pst_type} {pst_mapping.pst_id}.\n"
                        ...
                        f"(BC is {days_late} days late)\n\n"
                        f"Hint: BC may not meet delivery commitment for this PST."
                    ),
                    violations=[violation_msg]
                ))
        
        # Return results (empty list = all passed)
        if results:
            return results
