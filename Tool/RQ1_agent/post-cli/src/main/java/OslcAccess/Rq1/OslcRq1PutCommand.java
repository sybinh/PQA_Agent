...
class OslcRq1PutCommand extends OslcPutCommand {
    ...
    public OslcRq1PutCommand addPropertyField(String propertyPath, List<String> values) {
        assert (propertyPath != null);
        assert (propertyPath.isEmpty() == false);
        assert (values != null);
    ...
    public String buildBodyString() {
        ...
        for (OslcProperty property : writeProperties.getProperties()) {
            if (property instanceof OslcPropertyField) {
                ...
                List<String> values = ((OslcPropertyListField)property).getValues();
                if (values.isEmpty() == true) {
                    // An empty list field has also to be written because it means that the list has to be cleared
                    EcvXmlElement cq_fieldName = generateXmlElement(new OslcPropertyField(property.getName(), ""));
                    oslc_cm_ChangeRequest.addElement(cq_fieldName);
                } else {
