 ...
 * @author gug2wi
 */
public class Rule_IssueSW_FmeaCheck extends DmRule<DmRq1IssueSW> {
    ...
    public void executeRule() {
        ...
        if ((dmElement.isCanceledOrConflicted() == false) && (dmElement.SUBMIT_DATE.getValue().isLaterOrEqualThen(EcvDate.getDate(2015, 1, 1)))) {

            if (dmElement.FMEA_STATE.getValue() == FmeaState.NOT_REQUIRED) {
                if ((dmElement.FMEA_COMMENT.isEmpty() == true) && (dmElement.FMEA_CHANGE_COMMENT.isEmpty() == true)) {
                    addMarker(dmElement, new Warning(this, "Comment for not required FMEA missing.", "A comment has to be set when FMEA state is set to 'Not Required'."));
                }
            }
        }
