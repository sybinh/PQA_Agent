 ...
 * @author gug2wi
 */
public class Rule_Pvar_Derivatives extends DmRule<DmRq1PvarRelease> {
    ...
    public void executeRule() {
        ...
        Set<String> pilots = dmElement.DERIVATIVES.getValue().getMatchingDerivativeNames(EnumSet.of(Mode.PILOT));
        Set<String> mandatoryDerivatives = dmElement.DERIVATIVES.getValue().getMatchingDerivativeNames(EnumSet.of(Mode.MANDATORY));
        Map<String, EcvDate> plannedDates = dmElement.PLANNED_DATE_DERIVATIVES.getValue().getDates();

        //
        // Warn, if no pilot exists
        //
        if (pilots.isEmpty() == true) {
            addMarker(dmElement, new Warning(this, "No pilot derivative set.", "No derivative is set as pilot PVAR-Release."));
        }

        //
        // Warn, if more than one pilot exists
        //
        if (pilots.size() > 1) {
            ...
            addMarker(dmElement, new Warning(this, "Too many pilots set.", b.toString()));
        }
        ...
        if (pilots.size() == 1) {
            for (String pilot : pilots) {
                ...
                if (pilotDate == null) {
                    addMarker(dmElement, new Warning(this, "Planned date missing for pilot.", "No planned date set for pilot derivative " + pilot + "."));
                } else if (pilotDate.equals(dmElement.PLANNED_DATE.getValue()) == false) {
                    addMarker(dmElement, new Warning(this, "Planned date for pilot differs to planned date of release.",
                            "Planned date for pilot (" + pilotDate.toString() + ") differs to planned date of release (" + dmElement.PLANNED_DATE.getValue().toString() + ")."));
                }
            }
        }
        ...
        for (String mandatory : mandatoryDerivatives) {
            ...
            if (mandatoryDate == null) {
                addMarker(dmElement, new Warning(this, "Planned date missing for mandatory derivative.", "No planned date set for mandatory derivative " + mandatory + "."));
            } else if (mandatoryDate.isEarlierThen(dmElement.PLANNED_DATE.getValue()) == true) {
                addMarker(dmElement, new Warning(this, "Planned date for mandatory derivatives is earlier then planned date of release.",
                        "Planned date for derivative " + mandatory + " (" + mandatoryDate.toString() + ") is earlier then planned date of release (" + dmElement.PLANNED_DATE.getValue().toString() + ")."));
            }
        }
