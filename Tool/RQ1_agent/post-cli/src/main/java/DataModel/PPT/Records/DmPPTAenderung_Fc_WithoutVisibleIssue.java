 ...
 * @author gug2wi
 */
public class DmPPTAenderung_Fc_WithoutVisibleIssue extends DmPPTAenderung_Fc {

    //Dummy Issue at FC
    public DmPPTAenderung_Fc_WithoutVisibleIssue(DmPPTFCRelease fc) {
        super(fc);
        ...
        //Check the issueFDs from the bc and from the fc, if there are matching ones
        ArrayList<String> issueFDids = new ArrayList<>();
        for (DmRq1IssueFD issueFD : fc.RQ1_ISSUE_FDS.getElementList()) {
            if (fc.HAS_PARENT_RELEASE.getElement().RQ1_ISSUE_FDS.containsIssue(issueFD)) {
                issueFDids.add(issueFD.getId());
            }
        }
        String cti = "";
