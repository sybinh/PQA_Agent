 ...
 * @author gug2wi
 */
public class Rule_Rrm_Bc_Fc_DeliveryDate extends DmRule {
...
//    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(Rule_Rrm_Bc_Fc_DeliveryDate.class.getCanonicalName());
    ...
    final private DmRq1Rrm_Bc_Fc myRrm;
    ...
    public void executeRule() {
        ...
        DmRq1Bc bc = myRrm.MAPPED_PARENT.getElement();
        DmRq1Fc fc = myRrm.MAPPED_CHILDREN.getElement();
        ...
        if (fc.PLANNED_DATE.getValue().isEmpty() == true) {
            addMarker(myRrm, new Warning(this, "No planned date set for FC.", "No planned date set for FC."));
            return;
        }
        ...
        if (myRrm.REQUESTED_DELIVERY_DATE.getValue().isEmpty() == false) {
            // c)
            if (fc.PLANNED_DATE.getValue().isLaterThen(myRrm.REQUESTED_DELIVERY_DATE.getValue())) {
                addMarker(myRrm, new Warning(this, "FC planned date too late for BC requested delivery date.",
            ...
            if (fc.PLANNED_DATE.getValue().isLaterThen(bc.PLANNED_DATE.getValue())) {
                addMarker(myRrm, new Warning(this, "FC planned date too late for BC planned date.",
                        "PlannedDate of FC: " + fc.PLANNED_DATE.getValue().toString() + "\n"
