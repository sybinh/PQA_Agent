 ...
 * @author frt83wi
 */
public class Rule_Fc_Conflicted extends DmRule<DmRq1FcRelease> {
    ...
    static final public DmRq1RuleDescription description = new DmExcitedCpCRule(
            ...
            + "\n"
            + "Creates a warning if the FC is in \"Conflicted\" state and the date in "
            + "LifeCycleComment is either missing or in the past");

    public Rule_Fc_Conflicted(DmRq1FcRelease fcRelease) {
    ...
    public void executeRule() {
        ...
        // rule affects only conflicted FCs
        ...
        if (lastDueDate == null) {
            Warning warning = new Warning(this, "No due date found in life cycle state comment field. (QAM ID-1.3.0)", "LifeCycleState " + dmElement.LIFE_CYCLE_STATE.getValueAsText() + " FC does not contain a due date in life cycle state comment field.");
        ...
        if (lastDueDate.isInThePast()) {
            Warning warning = new Warning(this, "Due date found in life cycle state comment field is in the past. (QAM ID-1.3.0)", "LifeCycleState " + dmElement.LIFE_CYCLE_STATE.getValueAsText() + " FC due date in life cycle state comment field is in the past.");
            warning.addAffectedElement(dmElement);
            addMarker(dmElement, warning);
        }
    }
