 ...
 * @author frt83wi
 */
public class Rule_IssueFD_Conflicted extends DmRule<DmRq1IssueFD> {
    ...
    public Rule_IssueFD_Conflicted(DmRq1IssueFD issueFD) {
    ...
    public void executeRule() {
        ...
        // rule affects only conflicted I-FDs
        ...
        if (lastDueDate == null) {
            Warning warning = new Warning(this, "No due date found in life cycle state comment field. (QAM ID-1.3.0)", "LifeCycleState " + dmElement.LIFE_CYCLE_STATE.getValueAsText() + " Issue-FD does not contain a due date in life cycle state comment field.");
        ...
        if (lastDueDate.isInThePast()) {
            Warning warning = new Warning(this, "Due date found in life cycle state comment field is in the past. (QAM ID-1.3.0)", "LifeCycleState " + dmElement.LIFE_CYCLE_STATE.getValueAsText() + " Issue-FD due date in life cycle state comment field is in the past.");
            warning.addAffectedElement(dmElement);
            addMarker(dmElement, warning);
        }
    }
