/*
 * Copyright (c) 2009, 2021 Robert Bosch GmbH and its subsidiaries.
 * This program and the accompanying materials are made available under
 * the terms of the Bosch Internal Open Source License v4
 * which accompanies this distribution, and is available at
 * http://bios.intranet.bosch.com/bioslv4.txt
 */
package Rq1Cache.Fields;

import Ipe.Annotations.EcvElementList;
import Monitoring.RuleExecutionGroup;
import Rq1Cache.Monitoring.Rq1DataRule;
import Rq1Cache.Monitoring.Rq1ParseFieldException;
import Rq1Cache.Monitoring.Rq1UnexpectedDataFailure;
import Rq1Cache.Records.Rq1RecordInterface;
import Rq1Data.Monitoring.Rq1RuleDescription;
import Rq1Data.Types.Rq1DerivativeMapping;
import Rq1Data.Types.Rq1ParseException;
import java.util.EnumSet;

/**
 *
 * @author GUG2WI
 */
public class Rq1DatabaseField_DerivativeMapping extends Rq1DatabaseField_StringAccess<Rq1DerivativeMapping> implements Rq1FieldI<Rq1DerivativeMapping> {

    @EcvElementList("Rq1Data.Monitoring.Rq1RuleDescription")
    static final public Rq1RuleDescription derivativeMappingDesc = new Rq1RuleDescription(
            EnumSet.of(RuleExecutionGroup.RQ1_DATA),
            "Syntax of derivative mapping.",
            "Checks the syntax of the derivative mapping in an IRM, RRM or Release.\n"
            + "\n"
            + "The warning 'Unexpected data read from RQ1 database.' is set on the element, if the syntax is wrong. "
            + "The problem with the syntax is descriped in the warning.");

    private Rq1DataRule derivativeRule = null;

    public Rq1DatabaseField_DerivativeMapping(Rq1RecordInterface parent, String dbFieldName) {
        super(parent, dbFieldName, new Rq1DerivativeMapping());
    }

    @Override
    public boolean setOslcValue_Internal(String dbValue, Source source) {
        assert (dbValue != null);
        assert (source != null);

        removeMarker();

        Rq1DerivativeMapping mapping = new Rq1DerivativeMapping();
        try {
            mapping.setRq1Value(dbValue);
        } catch (Rq1ParseException ex) {
            StringBuilder s = new StringBuilder(50);
            s.append("Problem processing data from field ").append(getOslcPropertyName()).append(".");
            Rq1ParseFieldException containerEx = new Rq1ParseFieldException(s.toString(), ex);
            getParentRecord().setMarker(new Rq1UnexpectedDataFailure(getRule(), getParentRecord(), this, containerEx));
        }

        return (setDataSourceValue(mapping, source));
    }

    @Override
    protected String getOslcValue_Internal() {
        return (getDataSourceValue().provideValueAsStringForDb());
    }

    private Rq1DataRule getRule() {
        if (derivativeRule == null) {
            derivativeRule = new Rq1DataRule(derivativeMappingDesc);
        }
        return (derivativeRule);
    }

    private void removeMarker() {
        if (derivativeRule != null) {
            this.getParentRecord().removeMarkers(derivativeRule);
            derivativeRule = null;
        }
    }

    @Override
    protected void dbValueWasWrittenToDb() {
        removeMarker();
        super.dbValueWasWrittenToDb(); //To change body of generated methods, choose Tools | Templates.
    }
}
